...
def get_building_exposure(boundary, flood_map, building_gpkg):
    ...
    # Precompute counts in hexes for binning
    ...
    # Colorbar
    cb = plt.colorbar(hb, boundaries=bounds, spacing='proportional', ticks=bounds, extend='max',
                      shrink=0.5, aspect=25, pad=0.01)
    ...
    plt.gca().text(
        0.04, 0.98, f"Flooded buildings: {flooded_count}",
...
def getbuilding_exposure(huc_id, boundary=None, geeprojectID=None):
    ...
    if not building_gpkg.exists():
        msf.BuildingFootprintwithISO(countryISO, HUC_boundary, out_dir, geeprojectID)
